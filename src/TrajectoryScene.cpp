...
const float TARGET_EPSILON = 0.02f;
...
void addControlPointToActiveCube()
{
	if (cubes.empty())
	{
		return;
	...
	CubeInstance& cube = activeCube();
	cube.controlPoints.push_back(cube.position);
	if (cube.controlPoints.size() == 1)
	{
		cube.nextControlPointIndex = 0;
	}
	else if (cube.controlPoints.size() == 2)
	{
		cube.nextControlPointIndex = 1;
	}

	cout << "Objeto " << activeCubeIndex
		<< " - ponto " << cube.controlPoints.size()
		<< " salvo: ("
		<< cube.position.x << ", "
		<< cube.position.y << ", "
		<< cube.position.z << ")" << endl;
}

void updateTrajectory(CubeInstance& cube, float deltaTime)
{
	if (cube.controlPoints.size() < 2 || deltaTime <= 0.0f)
	{
		return;
	}

	if (cube.nextControlPointIndex >= cube.controlPoints.size())
	{
		cube.nextControlPointIndex = 0;
	}

	const glm::vec3 target = cube.controlPoints[cube.nextControlPointIndex];
	const glm::vec3 toTarget = target - cube.position;
	const float distance = glm::length(toTarget);
	const float maxStep = cube.trajectorySpeed * deltaTime;

	if (distance <= TARGET_EPSILON || distance <= maxStep)
	{
		cube.position = target;
		cube.nextControlPointIndex = (cube.nextControlPointIndex + 1) % cube.controlPoints.size();
		return;
	}

	cube.position += glm::normalize(toTarget) * maxStep;
}
...
void printControls()
{
	cout << "X, Y, Z: seleciona rotacao do objeto ativo no respectivo eixo" << endl;
	cout << "W, S: move no eixo Z (frente/tras)" << endl;
	cout << "A, D: move no eixo X (esquerda/direita)" << endl;
	cout << "I, J: move no eixo Y (cima/baixo)" << endl;
	cout << "[ e ]: escala uniforme (diminuir/aumentar)" << endl;
	cout << "P: salva a posicao atual como ponto da trajetoria do objeto ativo" << endl;
	cout << "N: cria novo objeto" << endl;
	cout << "TAB: alterna objeto ativo" << endl;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			updateTrajectory(cube, deltaTime);
		}
